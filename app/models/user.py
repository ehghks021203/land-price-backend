...
class User(Base):
    ...
    user_id = Column(
        Integer, primary_key=True, autoincrement=True, comment="사용자 고유 ID"
    )
    role = Column(
        Integer,
        nullable=False,
    ...
    phone_verified = Column(
        Boolean, nullable=False, default=False, comment="전화번호 인증 여부"
    )
    ...
    modified_at = Column(
        TIMESTAMP, server_default=func.now(), onupdate=func.now(), comment="수정일시"
    )
    last_login = Column(
        TIMESTAMP, server_default=func.now(), comment="마지막 로그인 일시"
    )
