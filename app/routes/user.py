...
async def reset_password(request: USER.ResetPasswordRequest, db: Session = Depends(get_db)):
  ...
  try:
    ...
    raise HTTPException(status_code=500, detail=f'메일 전송에 실패하였습니다.')
...
@user_router.get('/get-favorite-lands-by-user', response_model=USER.GetFavoriteLandsByUserResponse)
