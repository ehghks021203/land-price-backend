...
def code2addr(code: str, scale: int = 0, dict_format: bool = False) -> Union[str, Dict[str, str]]:
  ...
  sido, sigungu, eupmyeondong, donglee = match['sido'], match["sigungu"], match["eupmyeondong"], match["donglee"]
    
  if len(code) == 19:
    m = "" if code[10] == "1" else "산"
    main_n, sub_n = int(code[11:15]), int(code[15:19])
    detail = f"{main_n}-{sub_n}" if sub_n != 0 else str(main_n)
    full_address = " ".join(filter(None, [sido, sigungu, eupmyeondong, donglee, f"{m}{detail}"]))
  else:
